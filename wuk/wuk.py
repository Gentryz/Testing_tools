...
def start_webtest():
    try:
        ...
        logging.basicConfig(filename=os.path.join(os.getcwd(), "保留配置页面升级" + current_time + '.log.txt'),
                            level=logging.DEBUG)
        WEBshowinfo_info()
        UUTPWDshowinfo_info()
                ...
                try:
                        ...
                        if "Upload successful" in text:
                            ...
                            try:
                                ...
                                if "is-checked" in status:
                                    ...
                                    time.sleep(5)
                                    try:
                                        WebDriverWait(browser, 200, 0.5).until(
                                            lambda browser: browser.find_element(By.XPATH,"//input[@type='password']"))
                                    except Exception as e:
                                        status_label.configure(text="程序运行失败", bg="red")
                                        faild_count += 1
                                ...
                                result = browser.find_element(By.XPATH, "//span[@class='res-badge is-unpass']").text
                                showinfo("=" * 25 + "固件上传成功,上传结果：{}".format(result) + "=" * 25)
                                Reasons = browser.find_element(By.XPATH, "//*[@id='pane-local']/div/div[2]/div[1]").text
                                showinfo("=" * 25 + "{}".format(Reasons) + "=" * 25)
                                status_label.configure(text="程序运行失败", bg="red")
                                faild_count += 1
                                fail_ind.append(str(i + 1))
                    ...
                    if i + 1 < COUNT:
                        ...
                        showinfo("=" * 20 + "The {} loop ends,Waiting {} S sleep time".format(i + 1,
                                                                                              INTERVALT) + "=" * 20)
                        logging.info(
                            "=" * 30 + "The {} loop ends,Waiting {} S sleep time".format(i + 1,
                                                                                         INTERVALT) + "=" * 30)
