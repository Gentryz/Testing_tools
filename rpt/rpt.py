    ...
    if "Known Networks" in l:
        ...
        if WIFI_NAME in list:
            ...
            try:
                ...
                browser.find_element(By.XPATH, "//span[@class='iconfont icon-delete btn-icon']").click()
            time.sleep(3)
        else:
            time.sleep(1)
    else:
...
def repeater():
    try:
        ...
        logging.basicConfig(filename=os.path.join(os.getcwd(), "中继测试" + current_time + '.log.txt'),
                            level=logging.DEBUG)
        WEBshowinfo_info()
        UUTPWDshowinfo_info()
        ...
        for i in range(COUNT):
            ...
            if i + 1 < COUNT:
                success_count += 1
                showinfo("=" * 20+"中继成功,次數:{}".format(success_count)+ "=" * 20)
                logging.info("=" * 20+"中继成功,次數:{}".format(success_count)+ "=" * 20)
                ...
                browser.close()
                success_count += 1
                showinfo("中继成功,次數:{}".format(success_count))
                logging.info("中继成功,次數:{}".format(success_count))
