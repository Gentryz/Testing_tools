...
def Restore_button():
    ...
    PCIP_Input.configure(state=NORMAL)
    PCWIP_Input.configure(state=NORMAL)
    WIFIName_Input.configure(state=NORMAL)
    WIFIPWD_Input.configure(state=NORMAL)
    RUNT_Input.configure(state=NORMAL)
...
def StartTest():
    try:
        ...
        logging.basicConfig(filename=os.path.join(os.getcwd(), "不控制继电器上下电wifi跑流" + current_time + '.log.txt'),
                            level=logging.DEBUG)
        UPshowinfo_info()
        PCLshowinfo_info()
        PCWIPshowinfo_info()
